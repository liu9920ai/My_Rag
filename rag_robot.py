...
class RAGSystem:
    def __init__(
        ...
        # 初始化数据处理组件
        self.data_processor = DataProcessor(processed_dir=processed_dir)
    ...
    def _keep_db_connection_open(self):
        ...
        if self.data_processor.conn is None:
            self.data_processor.conn = sqlite3.connect(
                self.data_processor.output_dir / "segments.db"
            )
    ...
    def _retrieve_context(self, question: str) -> List[Dict]:
        ...
        distances, indices = self.index.search(
            np.array([query_embedding], dtype=np.float32), 5
        )
